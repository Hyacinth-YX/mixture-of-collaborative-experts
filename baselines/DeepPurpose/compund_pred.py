...
def model_pretrained(path_dir="pretrained", model=None):
...
def load_local_property_pretrained(name, path):
    from dgllife.model.pretrain.property_prediction import create_property_model
    model = create_property_model(name)
    ...
    print('Pretrained model loaded from local')
    return model
...
class DGL_GIN_AttrMasking(nn.Module):
    def __init__(self, predictor_dim=None):
        ...
        local_pretrained_path = f"{name}_pre_trained.pth"
        if osp.exists(local_pretrained_path):
            self.gnn = load_local_property_pretrained(name, local_pretrained_path)
        else:
            self.gnn = load_pretrained(name)
...
class Property_Prediction:
    ...
    def __init__(self, **config):
        ...
        if self.save and not os.path.exists(self.result_folder):
    ...
    def train(self, train, val, test=None, verbose=True):
        ...
        if test is not None:
            ...
            elif self.drug_encoding in ['DGL_GCN', 'DGL_GAT', 'DGL_NeuralFP', 'DGL_GIN_AttrMasking',
                                        'DGL_GIN_ContextPred', 'DGL_AttentiveFP']:
                params_test['collate_fn'] = dgl_collate_func
            testing_generator = data.DataLoader(
                data_process_loader_Property_Prediction(test.index.values, test.Label.values, test, **self.config),
                **params_test)
                ...
                if self.binary:
                    ...
                    if verbose:
                        print('Validation at Epoch ' + str(epo + 1) + ' , MSE: ' + str(mse)[
                              ...
                              + str(r2)[:7] + ' with p-value: ' + str(f"{p_val:.2E}") + ' , Concordance Index: ' + str(
                            CI)[:7])
            table.add_row(lst)
    ...
    def predict(self, df_data, verbose=True):
        ...
        if verbose:
            print('predicting...')
        info = data_process_loader_Property_Prediction(df_data.index.values, df_data.Label.values, df_data,
                                                       **self.config)
